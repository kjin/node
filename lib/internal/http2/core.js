...
function connectionListener(socket) {
  ...
  // Set up the Session
  var session = createServerSession(this[kOptions], socket, options.settings);
...
function createServerSession(options, socket, settings) {
  return new Http2Session(constants.NGHTTP2_SESSION_SERVER,
...
function connect(authority, options, listener) {
  ...
  const port = '' + (authority.port !== '' ? authority.port : 433);
