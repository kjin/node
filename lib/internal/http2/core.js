...
function onConnect() {
  ...
  const listener = this[kListener];
  const session = createClientSession(options, this);
  listener(session, this);
}

// connect(options, listener)
// connect(path, options, listener)
function connect(port, host, options, listener) {
  ...
  const socket = net.connect(port, host, onConnect);
  socket[kOptions] = options;
  socket[kListener] = listener;
}

function secureConnect(port, host, options, listener) {
  if (typeof host === 'function') {
    listener = host;
    host = undefined;
    options = new Holder();
